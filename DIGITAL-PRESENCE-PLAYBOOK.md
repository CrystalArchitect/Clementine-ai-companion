...
This file is **operational**, not canon. It lives at the repository root rather
than in `content/`, so it is not published to the site's document archive.
...
Built: `/ledger` on teraustralis.com.au (`src/routes/ledger/+page.svelte`,
entries in `src/lib/ledger.js`).
